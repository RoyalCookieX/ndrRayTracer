...
for %%l in (%_LIBS%) do set _LIB_FLAGS=!_LIB_FLAGS! %%l
...
for %%l in (%_LIB_DIRS%) do (
    ...
    set _LIB_DIR_FLAGS=!_LIB_DIR_FLAGS! LIBPATH:%%l
)
...
echo Building %_NAME%...
call cl /MP /nologo %_SOURCE_FILES% /Fo%_BIN_DIR% /std:c17 %_CONFIG_FLAGS% %_INCLUDE_DIR_FLAGS% %_DEFINE_FLAGS% /link /NOLOGO /OUT:%_BIN_DIR%%_NAME%.exe %_LIB_DIR_FLAGS% %_LIB_FLAGS%
